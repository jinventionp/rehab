    ...
    <table class="table table-sm mb-0">
        ...
        <tbody>
	        ...
	        <tr>
	            ...
	            <td><?= h($user->birth_date->i18nFormat('YYYY-MM-dd')) ?></td>
	            <td><?= h($user->active) ?></td>
                ...
                <td class="actions">
                    ...
                    <a id="deleteRecord-<?=$user->id?>"  href="javascript:void(0);" data-url="<?=$this->Url->build(["controller" => "Users", "action" => "delete", $user->id])?>" data-url-action="<?=$this->Url->build(["controller" => "Users", "action" => "delete", $user->id])?>" data-title="<?=__('Delete User')?>" data-id="<?=$user->id?>" data-question='Seguro que desea eliminar el <?=__('User')?> <span class="badge bg-soft-primary text-primary"><?=$user->name?></span> ?'  data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Eliminar" class="action-icon"> <i class="fas fa-trash-alt fa-xs"></i></a>
                </td>
	            <!--<td class="actions">
